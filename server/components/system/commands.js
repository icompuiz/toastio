...
function setGlobalUser(req, res, next) {
	function setUser() {
		components.statusService.session = req;
	}
...
function commandWhoami(req, res) {
	...
	if (req.query.noresources) {
		...
		Route.whatResources(id, function(err, resources) {
			...
			req.user.populate('mruClients', function(err, user) {
				console.log('expressRoutes::who::After populating mruClients');

				user = {
					_id: user._id,
					username: user.username,
					resources: resources,
					mruClients: user.mruClients || []
				};
